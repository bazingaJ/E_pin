...
#define MessageMAX_H    30.0f      //messagelab的最大高度，当超过时，文本会以...结尾
...
@property (nonatomic,strong)UIView *messageLab;
...
-(instancetype)initWithTitle:(NSString *)title message:(NSString *)message cancelBtnTitle:(NSString *)cancelTitle otherBtnTitle:(NSString *)otherBtnTitle clickIndexBlock:(LXAlertClickIndexBlock)block{
    if(self=[super init]){
        ...
        _messageLab=[[UIView alloc] init];
        _messageLab.backgroundColor=[UIColor whiteColor];
        CGFloat endMessageLabH =MessageMAX_H;
        ...
        UITextField *fild  = [[UITextField alloc]initWithFrame:CGRectMake(0, 0, _messageLab.frame.size.width-100, _messageLab.frame.size.height)];
        fild.borderStyle = UITextBorderStyleRoundedRect;
        fild.font = [UIFont systemFontOfSize:12];
        fild.placeholder = @"请输入验证码";

//        fild.textAlignment = NSTextAlignmentCenter;
        ...
        [countBtn setFrame:CGRectMake(fild.frame.origin.x +fild.frame.size.width, fild.frame.origin.y, _messageLab.frame.size.width-fild.frame.size.width, fild.frame.size.height)];
        [countBtn addTarget:self action:@selector(ClickM:) forControlEvents:UIControlEventTouchUpInside];
        [countBtn setTitle:@"获取验证码" forState:UIControlStateNormal];
        [countBtn setTitleColor:[UIColor redColor] forState:UIControlStateNormal];
        countBtn.titleLabel.font = [UIFont systemFontOfSize:14];

        [_messageLab addSubview:fild];
        [_messageLab addSubview:countBtn];
        
        //计算_alertView的高度
        _alertView.frame=CGRectMake(0, -100, AlertView_W, _messageLab.frame.size.height+LXATitle_H+LXABtn_H+40);
        _alertView.center=CGPointMake(EPScreenW/2, EPScreenH/2-50);
...
-(void)ClickM:(JKCountDownButton *)countBtn
{
    EPData * data = [EPData new];
    [data resetPasswordWithwithPostType:@"3" withPassword:nil withCodeNo:nil withNewPayPassword:nil withCompletion:^(NSString *returnCode, NSString *msg,NSMutableDictionary * dic) {
        NSLog(@"returncode====%@",returnCode);
        if ([returnCode intValue]==0) {
            [countBtn startWithTime:59 title:@"重新获取" countDownTitle:@"秒后重试" mainColor:RGBColor(75, 187, 245)  countColor:[UIColor whiteColor]];
        }
        NSLog(@"msg====%@",msg);
        }];
}
-(void)btnClick:(UIButton *)btn{
    ...
    if (self.clickBlock) {
        self.clickBlock(btn.tag,self.field.text);
    }
