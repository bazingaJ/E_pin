...
- (void)viewDidAppear:(BOOL)animated
{
    ...
    if (!LOGINTIME)
    {
        [EPTool addAlertViewInView:self title:@"温馨提示" message:@"您还没有登陆无法享受扫描付车资优惠" count:1 doWhat:^{
...
- (void)viewDidLoad
{
    [super viewDidLoad];
    [self openScanning];
    [self addNavigationBar:2 title:@"扫一扫"];
    [self addLeftItemWithFrame:CGRectZero textOrImage:0 action:@selector(onBackClick) name:@"返回"];
    [self addLightRightItemWithFrame:CGRectMake(EPScreenW-20-22,20,44,44) action:@selector(onOpenFlashClick:) name:@"light"];
}
-(void)addLightRightItemWithFrame:(CGRect)frame action:(SEL)action name:(NSString *)name
{
    UIButton *button = [UIButton buttonWithType:UIButtonTypeCustom];
    [button setFrame:frame];
    [button setImage:[UIImage imageNamed:@"light"] forState:UIControlStateNormal];
    [button setImage:[UIImage imageNamed:@"lightSelect"] forState:UIControlStateSelected];
